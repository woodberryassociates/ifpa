...
<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
		...
		<article style="display: block; min-height: 220px" id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<?php if (!$frontPage) {
				echo "<h2 class='thumbnail-article-text'"; // hack to correctly style the excerpt
				html5wp_excerpt('html5wp_index');
				echo "</h2><a style='display: flex; padding: 0px 10px;'";
				edit_post_link();
				echo "</a>";
			} ?>
			<!-- /excerpt, read more, edit post links -->
		...
		<?php $articleCount++; ?>
		<?php if ($frontPage && $articleCount <= 1) : // front page only has 5 articles, so hardcoding this
		?>
			<div class="divider" style="margin-bottom: 20px; height: 1px; width: 100%;"></div>
		<?php elseif (!$frontPage && $articleCount % get_option('posts_per_page') != 0 && $articleCount < $wp_query->found_posts) : ?>
			<div class="divider" style="margin-bottom: 20px; height: 1px; width: 100%;"></div>
		<?php endif; ?>
		<!-- /breaks between articles -->
